...
var store = {
  ...
  _dispatch: function _dispatch(prevState) {
    ...
    this._listeners.forEach(function (listener) {
      return listener(_this.state, prevState);
    });
  },
  state: {
  ...
  subscribe: function subscribe(listener) {
    ...
    return function () {
      return _this2._listeners.remove(listener);
    };
  },
...
function renderApp(state) {
  ...
  var soundBtnIcon = "#icon-sound-".concat(
    soundEnabledSelector() ? "on" : "off"
  );
...
var COLOR_CODES_W_INVIS = [].concat(_toConsumableArray(COLOR_CODES), [
  INVISIBLE,
...
function init() {
  ...
  setOptionsForSelect(
    ...
    [0.5, 0.62, 0.75, 0.9, 1.0, 1.5, 2.0].map(function (value) {
      return {
...
function update(frameTime, lag) {
  ...
  COLOR_CODES_W_INVIS.forEach(function (color) {
    ...
    for (var _i = sparks.length - 1; _i >= 0; _i = _i - 1) {
      var spark = sparks[_i];
      spark.life -= timeStep;
...
function render(speed) {
  ...
  trailsCtx.fillStyle = "rgba(0, 0, 0, ".concat(
  ...
  while (BurstFlash.active.length) {
    ...
    var burstGradient = trailsCtx.createRadialGradient(
      bf.x,
      bf.y,
      0,
      bf.x,
...
function colorSky(speed) {
  ...
  targetSkyColor.b =
    (targetSkyColor.b / maxColorComponent) * maxSkySaturation * intensity; // Animate changes to color to smooth out transitions.
  ...
  appNodes.canvasContainer.style.backgroundColor = "rgb("
    .concat(currentSkyColor.r | 0, ", ")
    .concat(currentSkyColor.g | 0, ", ")
    .concat(currentSkyColor.b | 0, ")");
}
...
function createBurst(count, particleFactory) {
  ...
  var arcLength =
    arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : PI_2;
  ...
  for (var i = 0; i <= C_HALF; i++) {
    ...
    for (var _i2 = 0; _i2 < partsPerArc; _i2++) {
      var randomAngleOffset = Math.random() * maxRandomAngleOffset;
      var angle = angleInc * _i2 + angleOffset + randomAngleOffset;
  ...
  _createClass(Shell, [
    {
      ...
      value: function launch(position, launchHeight) {
        ...
        comet.onDeath = function (comet) {
          return _this3.burst(comet.x, comet.y);
        };
    ...
    },
    {
      ...
      value: function burst(x, y) {
        ...
        var starFactory = function starFactory(angle, speedMult) {
          ...
          var standardInitialSpeed = _this4.spreadSize / 1800;
          var star = Star.add(
            x,
            y,
            color || randomColor(),
            angle,
            speedMult * speed, // add minor variation to star life
            _this4.starLife +
              Math.random() * _this4.starLife * _this4.starLifeVariation,
            _this4.horsetail ? _this4.comet && _this4.comet.speedX : 0,
            _this4.horsetail
              ? _this4.comet && _this4.comet.speedY
              : -standardInitialSpeed
          );

          if (_this4.secondColor) {
            star.transitionTime =
              _this4.starLife * (Math.random() * 0.05 + 0.32);
            star.secondColor = _this4.secondColor;
          }

          if (_this4.strobe) {
            star.transitionTime =
              _this4.starLife * (Math.random() * 0.08 + 0.46);
            ...
            if (_this4.strobeColor) {
              star.secondColor = _this4.strobeColor;
            }
          }
          ...
          if (_this4.glitter) {
            ...
            star.sparkColor = _this4.glitterColor;
        ...
        if (this.comet) {
          ...
          var maxDiff = 2;
          var sizeDifferenceFromMaxSize = Math.min(
            maxDiff,
...
var soundManager = {
  ...
  preload: function preload() {
    ...
    types.forEach(function (type) {
      var source = _this5.sources[type];
      var fileNames = source.fileNames;
      var filePromises = [];
      fileNames.forEach(function (fileName) {
        var fileURL = _this5.baseURL + fileName; // Promise will resolve with decoded audio buffer.
        ...
        var promise = fetch(fileURL)
          ...
          .then(function (data) {
            return new Promise(function (resolve) {
              _this5.ctx.decodeAudioData(data, resolve);
            });
          });
  ...
  playSound: function playSound(type) {
    ...
    var initialPlaybackRate = MyMath.random(
...
if (IS_HEADER) {
  ...
  setLoadingStatus("You can turn on sound 🔊 for better experience");
  setTimeout(function () {
